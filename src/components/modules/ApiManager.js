...
export default {
  ...
  getSong(objectId) {
    return fetch(`${remoteURL}/songs/${objectId}?_embed=chords&_embed=writers`).then(result =>
      result.json()
    )
  },
  ...
  getAllSongs(tableName, userId) {
    return fetch(
      `${remoteURL}/${tableName}?userId=${userId}&_sort=lastUpdated&_order=desc`
  ...
  checkUser(email, password) {
    return fetch(`${remoteURL}/users?email=${email}&password=${password}`).then(response =>
      response.json()
    )
  },
}
